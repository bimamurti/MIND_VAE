...
class Dataloader(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        if reduceddata is not None:
            self.data = reduceddata.data
            self.ncluster = reduceddata.ncluster
            self.idtrack = reduceddata.idtrack
            self.idtasks = reduceddata.idtasks
            self.scores = reduceddata.scores
        if device is None:
        ...
        with ProcessPoolExecutor(mp_context=multiprocessing.get_context("spawn"), max_workers=max_workers) as p:
            ...
            for fut in futures:
               # print('masuk b')
                item,nclu, idtrck,idtask,score = fut.result()
                if item is not None:
                    data.extend(item)
                    self.ncluster.extend(nclu)
                    self.idtrack.extend(idtrck)
                    self.idtasks.extend(idtask)
    ...
    def __getitem__(self, idx):
        return self.data[idx],self.ncluster[idx],self.idtrack[idx],self.idtasks[idx],self.scores[idx]

    @staticmethod
    def load(self, filename, inclusive_groups):
        if(self.reduceddata is not None):
            return self.data,self.ncluster,self.idtrack,self.idtasks,self.scores
        ...
        while tid0 < e-horizon:
            ...
            if len(idx):
                ...
                def neighbourtopktensor(self,agents,i,k):#i adalah index agennya
                    ...
                    curr=torch.tensor(agents[:,i][:, :2],device=device).unsqueeze(1)#curr adalah tensor posisi agent yang diamati sepanjang horizon, shape L x 1 x 2
                    others=torch.tensor(agents[:, :, :2] ,device=device)#others adalah tensor posisi semua agent sepanjang horizon, shape L x N x 2
                    ...
                    finalmean[i] = torch.inf
                    k_eff = min(k, max(agents.shape[1] - 1, 0))
                    if k_eff == 0:
                        return torch.empty((agents.shape[0], 0, agents.shape[2]), device="cpu")
                    top_values, top_indices= torch.topk(finalmean, k=k_eff,largest=False) 
                    #a=res<200
                    agents=torch.tensor(agents,device=device)#agents adalah tensor posisi semua agent sepanjang horizon, shape L x N x 2
        ...
        for hist, future, neighbor in traj:
            ...
            idtracks.append(idtrack+(self.idtask*10000000))
        ...
        return items,trajncluster,idtracks,idtasks,scorenorm#items = list of tuple (hist,future,neighbor) trajncluster = list of tuple (histncluster,futurencluster) idtracks = list of id track idtasks = list of id task scorenorm = list of normalized score
    ...
    def dtw(self,sequence_1, sequence_2):
        n, m = len(sequence_1), len(sequence_2)
        
        # Create the cost matrix (n+1 by m+1), initialized to infinity
        cost_matrix = np.inf * np.ones((n+1, m+1))
        
        # Initialize the starting point (0, 0) in the cost matrix
        cost_matrix[0, 0] = 0
        ...
        for i in range(1, n+1):
            for j in range(1, m+1):
                # Calculate the cost (Euclidean distance) between points
                cost = self.euclidean_distance(sequence_1[i-1], sequence_2[j-1])
                minim=cost + min(cost_matrix[i-1, j],    # Insertion
                                            cost_matrix[i, j-1],    # Deletion
                                            cost_matrix[i-1, j-1])
                # Update the cost matrix with the minimum cumulative cost
                cost_val=cost + minim
                ival=i
                jval=j
                if(cost_val<20000):#ganti threshold
                    cost_matrix[i, j] = cost_val
                else:
                    return cost_val

        # The DTW distance is the value at the bottom-right corner of the cost matrix
        dtw_distance = cost_matrix[ival, jval]
        ...
        # while i > 0 and j > 0:
        #     path.append((i-1, j-1))
        #     # Move to the direction of the minimum cost
    ...
    def frechet_distance(self,P, Q):
        ...
        N, M = len(P), len(Q)
        ca = torch.full((N, M), -1.0)
        def c(i, j):
            if ca[i, j] > -1:
                return ca[i, j]
            d = torch.linalg.norm(P[i] - Q[j])
            if i == 0 and j == 0:
                ca[i, j] = d
            elif i > 0 and j == 0:
