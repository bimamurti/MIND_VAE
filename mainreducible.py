...
parser.add_argument("--ckpt", type=str, default='log_eth/logrducibleloss23dectest')
...
#parser.add_argument("--prev_data",default='log_eth/logExRecplaynov28th9')
parser.add_argument("--prev_data",type=str)
parser.add_argument("--datacapacity",type=str,default="15")# check how capacity works
...
if __name__ == "__main__":
    ...
    def loadallpreviousmemories(folder_path):
        ...
        for folder in pt_folder:
            ...
            train_files = glob.glob(os.path.join(folder, "bestRLB.pt"))
            #print(pt_files)
            ...
            for batch, items in enumerate(train_data):
                ...
                if settings.prev_data!=None:
                    ...
                    for prev_data in allprevdata:
                        ...
                        #prevtrainingdata=prev_data[0].dataprevtrain
                        #print("rise!!!")
                        item,trainedindex=mergepreviousdatatraining(item,prevtrainingdata,batches,trainedindex)#harusnya diambil 20 persen jg dari batch nya, tapi sudah
                       # print("done!!")
                ...
                lossarray.append([item,errindividual,klindividual,torch.as_tensor(idtracks, dtype=torch.int, device=settings.device)])
        ...
        if losses is not None and settings.ckpt:
            ...
            if ade < ade_best:
                ...
                for i in range(len(lossarray)):
                    ...
                    temp=[lossarray[i][0][0][:,lossindices,:], lossarray[i][0][1][:,lossindices,:], lossarray[i][0][2][:, lossindices,:,:],lossarray[i][1][lossindices],lossarray[i][2][lossindices],lossarray[i][3][lossindices]]
                    if(i==0):
                        finalloss=temp
                    else:
                        ...
                        finalloss=[obv,fut,neighb,yt,ft,idt]                        
