...
if __name__ == "__main__":
   ...
   # print("ok1")
    ...
    bc = BCSR_Coreset(proxy_model=proxy_model, lr_proxy_model=settings.proxy_lr ,beta=10, out_dim=1,
            max_outer_it=1, max_inner_it=1,
            ...
            for batch, items in enumerate(train_data):
               ...
               # lossarray.append([item,errindividual,klindividual,torch.as_tensor(idtracks, dtype=torch.int, device=settings.device),torch.as_tensor(ncluster, dtype=torch.double, device=settings.device),torch.as_tensor(taskno, dtype=torch.int, device=settings.device)])
                ...
                if(epoch==end_epoch):#ini tentukan kapan training coresetnya
                    ...
                    n = item[0].shape[1]
                    weights = 1.0/n*torch.ones([n], dtype=torch.float, requires_grad=True)
    ...
    n_best=int(buffersize*0.007)
    offline_REL=offREL(settings=settings, config=config, model=beststate, lossmain=lossbest, buffer_size=buffersize, batch_size=batch_size, epoch=10,nbest=n_best)
