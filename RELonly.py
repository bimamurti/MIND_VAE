...
parser.add_argument("--ckpt", type=str, default='log_eth/logbilevel21janv1')
...
#parser.add_argument("--prev_data",default='log_eth/logbilevel5jantest4')
...
parser.add_argument("--taskno",type=str,default="0")
...
if __name__ == "__main__":
   ...
   # print("ok1")
    ...
    for epoch in range(start_epoch+1, end_epoch+1):
        ...
        if train_data is not None and epoch <= config.EPOCHS:
            ...
            for batch, items in enumerate(train_data):
                ...
                #     print('start coreset selection for batch ',batch)
                ...
                #     candidates_indices.append(ids.cpu().numpy())
                     ...
                     lastbatch.append([item_cpu,errindividual.clone().cpu(),klindividual.clone().cpu(),torch.as_tensor(idtracks, dtype=torch.int, device='cpu'),torch.as_tensor(ncluster, dtype=torch.double, device='cpu'),torch.as_tensor(idtask, dtype=torch.int, device='cpu')])
    ...
    offline_REL=offREL(settings=settings,model=model,data_train=train_dataset, config=config, lossmain=lossbest, buffer_size=buffersize, batch_size=batch_size, epoch=100,nbest=512)
    ...
    if(epoch==end_epoch):
        ...
        for i in range(len(lastbatch)):
            ...
            tempbatch=[lastbatch[i][0][0][:,batch_indices,:], lastbatch[i][0][1][:,batch_indices,:], lastbatch[i][0][2][:, batch_indices,:,:],lastbatch[i][1][batch_indices],lastbatch[i][2][batch_indices],lastbatch[i][3][batch_indices],lastbatch[i][4][batch_indices],lastbatch[i][5][batch_indices]]
            if(isnotempty==False and len(batch_indices)!=0):
                ...
                if(len(adeindices)==0 ):                       
                    ...
                    finalcand=[obv,fut,neighb,yt,ft,idtracks,ncluster,idtask]                        
