...
from utils import ADE_FDE, FPC, seed, get_rng_state, set_rng_state, randomsampletensor
from ContinualLearning.previousdata import previousdata
...
parser.add_argument("--train", nargs='+', default=['data/MOT214CL1Cdata/train'])#data/MOT212CL/train data/MOT17CL/train
parser.add_argument("--test", nargs='+', default=['data/MOT214CL1Cdata/test'])
#parser.add_argument("--train", nargs='+', default=['data/continualdata/dataMOT212/train'])#data/MOT212CL/train data/MOT17CL/train
#parser.add_argument("--test", nargs='+', default=['data/continualdata/dataMOT212CL/test'])
...
parser.add_argument("--ckpt", type=str, default='log_eth/logdataMOT214singlefull2')
...
parser.add_argument("--datacapacity",type=str,default="10")
   ...
   # print("ok1")
    ...
    def mergeprevioustestdata(test_dataset,prevtestdata,sample):
        ...
        for idx,rs in enumerate(prevtestdata):
            ...
            for iddata,datas in enumerate(rs):
                if(idx==0):    
    ...
    if settings.test:
        ...
        if settings.prev_data!=None:
            ...
            for prev_data in allprevdata:
                ...
                test_dataset=mergeprevioustestdata2(test_dataset,prevtestdata)
    ...
    def mergepreviousdatatraining(traindata,prevtrainingdata,sample):
        ...
        s=sample
        if(settings.method!="experiencereplay"):
            sample=int((sample/100)*len(traindata))
            random_samples=randomsampletensor(prevtrainingdata,weights=yt,k=sample)
            #random_samples = np.random.choice(prevtrainingdata,size=sample,replace=False,p=yt)
        else:
            #cpu_data = prevtrainingdata.cpu().numpy()
            sample=int((sample/100)*traindata[0].shape[1])
            traincount=traindata[0].shape[1]
            w = [1]*len(prevtrainingdata)
            random_samples,idx=randomsampletensor(prevtrainingdata,weights=w,k=sample)
      ...
      #  print('before',mem_used_MB)
        ...
        return [traindata0,traindata1,traindata2]
            ...
            for batch, item in enumerate(train_data):
                if settings.prev_data!=None:
                    ...
                    for prev_data in allprevdata:
                        ...
                        prevtrainingdata=prev_data[0].dataprevtrain
                        #print("rise!!!")
                        item=mergepreviousdatatraining(item,prevtrainingdata,sample)#harusnya diambil 20 persen jg dari batch nya, tapi sudah
                       # print("done!!")
                res = model(*item)
    ...
    max_mem = torch.cuda.max_memory_allocated(2)
