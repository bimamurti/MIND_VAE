...
class SocialVAE(torch.nn.Module):
    ...
    class P_Z(torch.nn.Module):
        ...
        def forward(self, x):
            ...
            loc = self.mu(x)
            std = self.std(x)
            # Clamp std to prevent underflow and ensure constraint > 0
    ...
    def __init__(self, horizon, ob_radius, hidden_dim=256):
        super().__init__()
    ...
    def enc(self, x, neighbor, *, y=None):
        ...
        with torch.no_grad():
            ...
            #print('distance!!!!!')
            #print(dist)
            mask = dist <= 10000#self.ob_radius
            ...
            features = torch.stack((dist, bearing, mpd), -1)# L x N x Nn x 3 b 

        k = self.embed_k(features)                          # L x N x Nn x d k adalah fitur sosial yang sudah di embed untuk digunakan di attention, s adalah fitur individu yang sudah di embed untuk digunakan di RNN dan juga untuk digabung dengan hasil attention untuk input ke RNN selanjutnya, n adalah fitur sosial yang sudah di embed untuk digunakan di gabung dengan s untuk input ke RNN selanjutnya
        ...
        h = h.view(N, -1, self.rnn_fx.num_layers)           #h adalah hidden state dari RNN yang digunakan untuk input ke RNN selanjutnya dan juga untuk input ke distribusi z, h diinisialisasi dengan fitur sosial dari neighbor terdekat pada timestep pertama
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.training:
            ...
            preverr=kwargs["err"] if "err" in kwargs else next(args)
            prevkl=kwargs["kl"] if "kl" in kwargs else next(args)
            #cluster=kwargs["cluster"] if "cluster" in kwargs else next(args)
            #score=kwargs["score"] if "score" in kwargs else next(args)
            return self.learn(x, y, neighbor, preverr=preverr,prevkl=prevkl)#, score=score, preverr=preverr,prevkl=prevkl)
    ...
    def learn(self, x, y, neighbor=None, score=None, map=None, preverr=None,prevkl=None):
        C = x.dim()
        if C < 3:
            x = x.unsqueeze(1)
            neighbor = neighbor.unsqueeze(1)
        ...
        pred = torch.cumsum(d, 0)
        preverror=preverr#dibuat perkalian bobot untuk nilai errornya jadi 1*preverror untuk semua data nya perorangnya sehingga nanti semua koordinat di kalikan dengan data perseorangnyaa. hanya saja bagaimana menyederahankan nilainya menjadi lebih rendah
        err = (pred - y).square()     
        kl = []
        for p, q, z in zip(P, Q, Z):
            kl.append(q.log_prob(z) - p.log_prob(z))
        kl = torch.stack(kl)
        resultscore = err.mean(dim=(0, 2))
        # if(score is not None):
        #     score_tensor = torch.tensor(score, device=err.device, dtype=err.dtype)
        #     resloss = resultscore * (score_tensor + 1) 
        return err, kl#,resloss
    def loss(self, err, kl):#, resloss):
        rec = err.mean()
        kl = kl.mean()
        #rec=resloss.mean()
        return {
