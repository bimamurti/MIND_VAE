...
parser.add_argument("--ckpt", type=str, default='log_eth/testexpuniform3')
...
#parser.add_argument("--prev_data",default='log_eth/testexpuniform2')
    ...
    while epoch < end_epoch:
        ...
        if train_data is not None and epoch <= config.EPOCHS:
            ...
            for batch, items in enumerate(train_data):
                ...
                idtask=items[-1]
                item=items[:-2]
                score = torch.ones(len(idtask), dtype=torch.float32)
                #a=ncluster[0]
                #print(a)
                if settings.prev_data!=None:
        ...
        if losses is not None and settings.ckpt:
            ...
            if ade < ade_best:
                ...
                for i in range(len(lossarray)):
                    ...
                    keep_prob = max(0.0, min(1.0, float(settings.datacapacity) / 100.0))#persentase kapasitas
                    k = max(1, int(round(num_samples * keep_prob)))#jumlah sampel yang akan dipilih
                    if k >= num_samples:
                        lossfinal = torch.ones(num_samples, dtype=torch.bool)
                    else:
                        perm = torch.randperm(num_samples)
                        selected = perm[:k]
                        lossfinal = torch.zeros(num_samples, dtype=torch.bool)
                        lossfinal[selected] = True
                    # match device of loss tensor
                    lossfinal = lossfinal.to(lossarray[i][1].device)
                    lossindices = torch.nonzero(lossfinal).squeeze()
                    temp=[lossarray[i][0][0][:,lossindices,:], lossarray[i][0][1][:,lossindices,:], lossarray[i][0][2][:, lossindices,:,:],lossarray[i][1][lossindices],lossarray[i][2][lossindices],lossarray[i][3][lossindices],lossarray[i][4][lossindices],lossarray[i][5][lossindices], lossarray[i][6][lossindices]]
                    if(i==0):
                        finalloss=temp
                    else:
                        ...
                        score=torch.cat((finalloss[8],temp[8]),dim=0)
                        finalloss=[obv,fut,neighb,yt,ft,idtracks,ncluster,idtask,score]                        
