...
from offREL import *
from bilevel_coreset import *

def socialvae_batch_to_2d_numpy(batch_tensor):
    if torch.is_tensor(batch_tensor):
        batch_tensor = batch_tensor.detach().cpu().numpy()
    batch_tensor = np.asarray(batch_tensor)
    if batch_tensor.ndim >= 3:
        batch_tensor = np.moveaxis(batch_tensor, 1, 0)
    return batch_tensor.reshape(batch_tensor.shape[0], -1).astype(np.float32)


def rbf_kernel_np(x, y, gamma=None):

    def to_samples(a):
        # support torch tensors (including CUDA tensors) and numpy-like inputs
        if torch.is_tensor(a):
            a = a.detach().cpu().numpy()
        a = np.asarray(a, dtype=np.float32)
        if a.ndim == 1:
            a = a.reshape(1, -1)
        # if features are rows and samples are columns, transpose to (n_samples, features)
    ...
    def stack(z):
        if isinstance(z, (list, tuple)):
            parts = [to_samples(p) for p in z]
            return np.concatenate(parts, axis=1)  # concat features
        return to_samples(z)
    ...
    sq_dist = np.maximum(x_norm + y_norm - 2.0 * X @ Y.T, 0.0)
    return np.exp(-gamma * sq_dist).astype(np.float32)
...
parser.add_argument("--ckpt", type=str, default='log_eth/testbcsr3')
...
#parser.add_argument("--prev_data",default='log_eth/testbc')
...
if __name__ == "__main__":
   ...
   # print("ok1")
    ...
    def weighted_mse(K, alpha, y, weights, lmbda):
        ...
        return loss
    bc = BilevelCoreset(outer_loss_fn=weighted_mse,
                                inner_loss_fn=weighted_mse, out_dim=config.PRED_HORIZON * 2, max_outer_it=5,
        ...
        if train_data is not None and epoch <= config.EPOCHS:
            ...
            for batch, items in enumerate(train_data):
                ...
                if(epoch==end_epoch):#ini tentukan kapan training coresetnya
                    ...
                    # pass the raw batch (trajectory, neighbour, future) and let the coreset
                    # handler flatten/concatenate appropriately
                    coreset_inds, coreset_weights, _ = bc.build_with_representer_proxy_batch(
                        (item[0], item[1], item[2]), 5, kernel_fn_np=rbf_kernel_np,
                        cache_kernel=True, start_size=3, inner_reg=1e-7)
                    ...
                    candidates_indices.append(np.asarray(coreset_inds))
                    item_cpu = tuple(t.clone().cpu() if torch.is_tensor(t) else t for t in item)
                    lastbatch.append([item_cpu,errindividual.clone().cpu(),klindividual.clone().cpu(),torch.as_tensor(idtracks, dtype=torch.int, device='cpu'),torch.as_tensor(ncluster, dtype=torch.double, device='cpu'),torch.as_tensor(idtask, dtype=torch.int, device='cpu'),torch.as_tensor(coreset_weights, dtype=torch.float32, device='cpu')])
