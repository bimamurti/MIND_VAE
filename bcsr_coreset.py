...
class BCSR_Coreset:
    ...
    def coreset_select(self, model, item, task_id,  topk, out_loss=None, ref_x=None, ref_y=None,idt=None):
        ...
        #     X = torch.from_numpy(X).float()
        n = item[0].shape[1]
        self.training_model_op.proxy_model.load_state_dict(model.state_dict())#penyamaan parameters
        
        print("proxy model initialized")# initialize sample weights
        coreset_weights = 1.0/n*torch.ones([n], dtype=torch.float, requires_grad=True)
        ...
        for i in range(self.max_outer_it):
            inner_loss = self.training_model_op.train_inner(item, task_id, coreset_weights, self.max_inner_it)
            coreset_weights, _, outer_loss = self.training_model_op.train_outer(item,task_id, coreset_weights, topk, ref_x, ref_y)
            coreset_weights = self.projection_onto_simplex(coreset_weights)
            total_loss = torch.mean(outer_loss).item()
        #print('inner loss:{:.3f}, outer loss:{:.3f}'.format(inner_loss["loss"], total_loss))
        if out_loss != None and n==50:
            out_loss.append(total_loss)
        idx = torch.multinomial(coreset_weights, topk, replacement=False)
        # Convert idt (list or tensor) to tensor on the same device as idx, then index
        #idt_tensor = torch.as_tensor(idt, device=idx.device)
        #ids = idt_tensor[idx]
        return idx, out_loss, coreset_weights
    
    def coreset_select_new(self, model, item, task_id,  topk, out_loss=None, ref_x=None, ref_y=None,idt=None):
        np.random.seed(self.seed)
        # if isinstance(y, np.ndarray):
        #     y = torch.from_numpy(y).float()
        # if isinstance(X, np.ndarray):
